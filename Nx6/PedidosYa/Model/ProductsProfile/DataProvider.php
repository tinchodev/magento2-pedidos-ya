...
namespace Nx6\PedidosYa\Model\ProductsProfile;

use Magento\Framework\App\Request\DataPersistorInterface;
use Magento\Framework\Registry;
use Magento\Ui\DataProvider\AbstractDataProvider;
use Magento\Ui\DataProvider\Modifier\PoolInterface;
use Nx6\PedidosYa\Model\ResourceModel\ProductsProfile\CollectionFactory;
...
class DataProvider extends AbstractDataProvider
{
    ...
    public function getData(): array
    {
        ...
        /** @var \Nx6\PedidosYa\Model\ProductsProfile $model */
        ...
        $persisted = $this->dataPersistor->get('nx6_pedidosya_products_profile');
        if (!empty($persisted)) {
            // Save persists the raw POST, whose primary field is products_profile_id; the lookup key
            // above must match the "id" request param, which is '' for a new record.
            unset($persisted['sftp_password']);
            $index = $persisted['products_profile_id'] ?? '';
            $this->loadedData[$index] = $persisted;
            $this->dataPersistor->clear('nx6_pedidosya_products_profile');
        }
